...
function fmov(state: SH4State, data: string) {
  ...
  if (options.pairwiseSingle) {
    return instructionResult(
      `${dest} = ${src} pairwise copy in SZ=1 mode`,
      access,
    );
  }
...
function setupArithmatic(
  ...
  if (srcIsDouble != destIsDouble) {
    throw new Error(
      `FMOV cannot mix single and double registers (got ${src}, ${dest})`,
    );
  }
...
function fsca(state: SH4State, data: string) {
  ...
  return instructionResult(
    `FR${base} = sin(2.0*PI*FPUL/65536) -> ${bank[base]}, FR${base + 1} = cos(2.0*PI*FPUL/65536) -> ${bank[base + 1]}`,
    {
      frWrite: [base, base + 1],
      fpulRead: true,
    },
...
function fipr(state: SH4State, data: string) {
  ...
  if (base0 % 4 != 0 || base1 % 4 != 0) {
    throw new Error(
      `FIPR requires FVm and FVn to be multiples of 4 (got ${base0}, ${base1})`,
    );
  }
  ...
  const res =
    bank[base0]! * bank[base1]! +
    bank[base0 + 1]! * bank[base1 + 1]! +
    bank[base0 + 2]! * bank[base1 + 2]! +
    bank[base0 + 3]! * bank[base1 + 3]!;
  bank[base1 + 3] = res;
  return instructionResult(`${data} = fv${base0} FIPR fv${base1} -> ${res}`, {
    frRead: [...registerRange(base0, 4), ...registerRange(base1, 4)],
    frWrite: [base1 + 3],
  });
...
function fmac(state: SH4State, data: string) {
  ...
  if (operands[0]!.trim().toUpperCase() != "FR0") {
    throw new Error(
      `FMAC requires FR0 as the first operand (got ${operands[0]})`,
    );
  }
